...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    plt.title("Freight Value vs Product Weight Relationship", fontsize=14)
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    for holiday in df[df["holiday"]]["date"]:
        plt.axvline(x=holiday, color='blue', linestyle='--',alpha = 0.5)
